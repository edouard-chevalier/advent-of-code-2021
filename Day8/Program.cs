...
namespace Day7
{
    class Program
    {
        private const string inputTest = @"be cfbegad cbdgef fgaecd cgeb fdcge agebfd fecdb fabcd edb | fdgacbe cefdb cefbgd gcbe
...
gcafb gcf dcaebfg ecagb gf abcdeg gaef cafbge fdbac fegbdc | fgae cfgab fg bagce";
        ...
        private const string input = @"dac abcf ac fdbcga dgcbae gcbfde fgcbd agfed adcgf cdbgfea | cbfa bcafdg cbfa bafcgd
...
";
        ...
        static void exo1()
            {
                ...
                var strings = input.Split("\r\n", StringSplitOptions.RemoveEmptyEntries).Select(s => s.Trim())
                    .ToArray();
                var digits = strings.Select(s => s.Split("|", StringSplitOptions.TrimEntries)[1])
                    .SelectMany(s => s.Split(" ", StringSplitOptions.RemoveEmptyEntries)).ToArray();
                Console.WriteLine(digits.Count(s => s.Length == 3 || s.Length == 7 || s.Length == 2 || s.Length == 4));
        ...
        static int Decode(string display)
        {
            ...
            static void exo2()
            {
                ...
                var strings = input.Split("\r\n", StringSplitOptions.RemoveEmptyEntries).Select(s => s.Trim())
                    .ToArray();
                var numbers = strings.Select(Decode).ToArray();
