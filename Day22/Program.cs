...
namespace Day22
{
    class Program
    {
        private const string inputTest = @"on x=-20..26,y=-36..17,z=-47..7
...
on x=967..23432,y=45373..81175,z=27513..53682";
        ...
        private const string inputTest2 = @"on x=-5..47,y=-31..22,z=-19..33
...
off x=-93533..-4276,y=-16170..68771,z=-104985..-24507";
        ...
        private const string input = @"on x=-37..10,y=-38..8,z=-18..35
...
";
        ...
        private static ( int min, int max) ToRange(string s)
        {
            var tmp = s.Split("..", StringSplitOptions.RemoveEmptyEntries);
            return (Int32.Parse(tmp[0]), Int32.Parse(tmp[1]));
        }


        private static ( int min, int max)[] ToRanges(string s)
        {
            var tmp = s.Split(",", StringSplitOptions.RemoveEmptyEntries);
            return tmp.Select(l => ToRange(l.Substring(2))).ToArray();
        }
        ...
        static void exo1()
        {
            ...
            {
                var tmp = step.Split(" ", StringSplitOptions.TrimEntries);
                var ranges = ToRanges(tmp[1]);
                bool turnOn = tmp[0] == "on";
        ...
        /* List<(int min, int max)> ToSplitintervals((int min, int max)[] cuts)
         {
             Array.Sort(cuts, (a, b) => a.min - b.min);
             var current = cuts[0];

             var res = new List<(int min, int max)>();
             ...
             while( true)
             {
                 ...
                 {
                     res.Add((current.min, current.min));
                     var tmpCurrent = (current.min + 1, Math.Min(current.max, next.max));
                     next = ()
                 }

                 res.Add((current.min, next.min -1));
                 current = (next.min, Math.Min(current.max))
        ...
        public static bool IntersectRange((int min, int max) a, (int min, int max) b)
        {
            if (a.min > b.min)
            {
                return b.max >= a.min;
        ...
        public class Cube
        {
            ...
            {
                var tmp = s.Split(",", StringSplitOptions.RemoveEmptyEntries);
                var ranges = tmp.Select(l => ToRange(l.Substring(2))).ToArray();
                X = ranges[0];
                Y = ranges[1];
                Z = ranges[2];
            }

            public Cube((int Min, int Max) x, (int Min, int Max) y, (int Min, int Max) z)
            {
                X = x;
                Y = y;
                Z = z;
            ...
            public ( int Min, int Max) Z { get; set; }

            public bool Contains(Cube other)
            {
                return (other.X.Min >= X.Min) && (other.X.Max <= X.Max) && (other.Y.Min >= Y.Min) &&
                       (other.Y.Max <= Y.Max) && (other.Z.Min >= Z.Min) && (other.Z.Max <= Z.Max);
            }

            public bool ContainsX((int min, int max) other)
            {
                return (other.min >= X.Min) && (other.max <= X.Max);
            }

            public bool ContainsY((int min, int max) other)
            {
                return (other.min >= Y.Min) && (other.max <= Y.Max);
            }

            public bool ContainsZ((int min, int max) other)
            {
                return (other.min >= Z.Min) && (other.max <= Z.Max);
            }

            public bool Intersect(Cube other)
            {
                return IntersectRange(other.X, X) && IntersectRange(other.Y, Y) && IntersectRange(other.Z, Z);
            }

            public long NbCubes =>
                ((long)(X.Max - X.Min + 1)) * ((long)(Y.Max - Y.Min + 1)) * ((long)(Z.Max - Z.Min + 1));
        }

        public static long NbCubes((int Min, int Max) X, (int Min, int Max) Y, (int Min, int Max) Z)
        {
            return ((long)(X.Max - X.Min + 1)) * ((long)(Y.Max - Y.Min + 1)) * ((long)(Z.Max - Z.Min + 1));
        }


        public class StepForest
        {

            public List<Step> Steps = new List<Step>();

            public StepForest(Step initialStep)
            {
                Steps.Add(initialStep);
            }

            public bool CanBeAdded(Step step)
            {
                return Steps.Any(s => step.Cube.Intersect(s.Cube));
            }
        ...
        {
            public Step(bool @on, Cube cube, int idx)
            {
                On = @on;
                Cube = cube;
                Idx = idx;
        ...
        static void exo2()
        {
            ...
            {
                var step = strings[index];
                var tmp = step.Split(" ", StringSplitOptions.TrimEntries);
                steps.Add(new Step(tmp[0] == "on", new Cube(tmp[1]), index));
            }

            List<StepForest> stepForests = new List<StepForest>();
            foreach (var step in steps)
            {
                var forest = stepForests.FirstOrDefault(f => f.CanBeAdded(step));
                if (forest != null)
                {
                    forest.Steps.Add(step);
                }
                else
                {
                    stepForests.Add(new StepForest(step));
                }
            }


            var xCuts = steps.SelectMany(s => new[] { s.Cube.X.Min, s.Cube.X.Max }).Distinct().ToArray();
            Array.Sort(xCuts);
            var yCuts = steps.SelectMany(s => new[] { s.Cube.Y.Min, s.Cube.Y.Max }).Distinct().ToArray();
            Array.Sort(yCuts);
            var zCuts = steps.SelectMany(s => new[] { s.Cube.Z.Min, s.Cube.Z.Max }).Distinct().ToArray();
            Array.Sort(zCuts);

            long total = 0;

            void CountRebootForCube(Cube c, List<Step> stepsZ)
            {
                bool on = false;
                foreach (var step in stepsZ)
                {
                    if (step.Cube.ContainsZ(c.Z))
                    {
                        on = step.On;
                    }
                }
            ...
            bool IsCubeOn((int min, int max) z, List<Step> currentSteps)
            {
                bool on = false;
                foreach (var step in currentSteps)
                {
                    if (step.Cube.ContainsZ(z))
                    {
                        on = step.On;
                    }
                }
            ...
            List<(int min, int max)> ToIntervals(int[] pos)
            {
                var res = new List<(int min, int max)>();
                res.Add((pos[0], pos[0]));
            ...
            var xRanges = ToIntervals(xCuts);
            var yRanges = ToIntervals(yCuts);
            var zRanges = ToIntervals(zCuts);

            var totalCubesToTest = (long) xRanges.Count * yRanges.Count * zRanges.Count;
            Console.WriteLine($"Will test{ totalCubesToTest} cubes");
            long count = 0;
            var xSteps = new List<Step>(steps.Count);
            var ySteps = new List<Step>(steps.Count);
            var zSteps = new List<Step>(steps.Count);
            foreach (var xRange in xRanges)
            {
                xSteps.Clear();
                foreach (var step in steps)
                {
                    if (step.Cube.ContainsX(xRange))
                    {
                        xSteps.Add(step);
                    }
                }
                ...
                {
                    ySteps.Clear();
                    foreach (var step in xSteps)
                    {
                        if (step.Cube.ContainsY(yRange))
                        {
                            ySteps.Add(step);
                        }
                    }
                    foreach (var zRange in zRanges)
                    {
                        ...
                        if (IsCubeOn(zRange, ySteps))
                        {
                            total += NbCubes(xRange, yRange, zRange);
                        }
                        count++;
                    }
